use alloy_primitives::{Address, B256, Log, U256, aliases::BlockNumber, b256};
    ...
    fn get_transfers(
        ...
        for bn in *block_start..=*block_end {
            ...
            info!(
                "Block {} has {} txns",
                bn,
                txns_in_block.tx_num_range().count()
            );

            for tx_num in txns_in_block.tx_num_range() {
                let transaction_data_debug = provider
                    .transaction_by_id(tx_num)
                    .context("failed to get transaction")?
                    .context(format!("No transaction found for tx_num {:?}", tx_num))?;

                let transaction_hash_debug = transaction_data_debug.hash();
                debug!("Transaction hash: {}", transaction_hash_debug);
                ...
                for log in &tx_receipt.logs {
                    ...
                    if log.topics().len() == 3 && log.topics()[0] == ERC20_TRANSFER_EVENT_SIGNATURE
                    {
                        ...
                        debug!("Decoded address was {:?}", from);
